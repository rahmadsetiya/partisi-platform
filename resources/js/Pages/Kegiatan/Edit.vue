...
const form = useForm({
    ...
    jenis: props.kegiatan.jenis,
    tahun: props.kegiatan.tahun,
    gelombang: props.kegiatan.gelombang ?? '',
    tanggal_mulai: props.kegiatan.tanggal_mulai,
    tanggal_selesai: props.kegiatan.tanggal_selesai ?? '',
    deskripsi: props.kegiatan.deskripsi ?? '',
});

function submit() {
    form.patch(route('kegiatan.update', props.kegiatan.id));
}
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold leading-tight text-gray-800">Edit Kegiatan</h2>
        </template>
